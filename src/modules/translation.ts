...
export class TranslationAPI {
  ...
  public static init(config: TranslationConfig): void {
    ...
    if (typeof ztoolkit !== "undefined") {
      ztoolkit.log("Paper Copilot: Translation API initialized with provider:", config.provider);
    }
  }
  ...
  public static loadFromPrefs(): boolean {
    try {
      const provider = Zotero.Prefs.get("paper-copilot.translation-provider") as string;
      const apiKey = Zotero.Prefs.get("paper-copilot.translation-api-key") as string;
      const targetLanguage = Zotero.Prefs.get("paper-copilot.translation-target-language") as string;
      ...
      if (provider && targetLanguage) {
        this.config = {
          ...
          apiKey: apiKey || "",
          targetLanguage,
          sourceLanguage: Zotero.Prefs.get("paper-copilot.translation-source-language") as string || "auto",
        };
  ...
  public static saveToPrefs(config: TranslationConfig): void {
    try {
      ...
      Zotero.Prefs.set("paper-copilot.translation-target-language", config.targetLanguage);
      if (config.sourceLanguage) {
        Zotero.Prefs.set("paper-copilot.translation-source-language", config.sourceLanguage);
      }
  ...
  public static async translate(
    ...
    if (this.config!.provider === "deepl") {
      return this.deeplTranslate(text, targetLanguage, sourceLanguage, options?.stream);
    } else if (this.config!.provider === "google") {
      return this.googleTranslate(text, targetLanguage, sourceLanguage, options?.stream);
    } else {
      return this.llmTranslate(text, targetLanguage, sourceLanguage, options?.stream);
    }
  }
  ...
  private static async deeplTranslate(
    ...
    const sourceLang = sourceLanguage === "auto" ? "" : (DEEPL_LANGUAGE_MAP[sourceLanguage] || sourceLanguage);
  ...
  private static async googleTranslate(
    ...
    const sourceLang = sourceLanguage === "auto" ? "auto" : (sourceLanguage === "EN" ? "en" : sourceLanguage.toLowerCase());
  ...
  private static async llmTranslate(
    ...
    let sourceLangName = sourceLanguage === "auto" ? "the original language" : this.getLanguageName(sourceLanguage);
...
4. Only output the translated text, nothing else`;
  ...
  private static getLanguageName(code: string): string {
    const lang = SUPPORTED_LANGUAGES.find(l => l.code === code || l.code.toLowerCase() === code.toLowerCase());
