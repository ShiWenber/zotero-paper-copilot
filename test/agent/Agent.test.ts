...
describe("Agent", function () {
  ...
  describe("handleToolCalls", function () {
    it("should execute a tool call", async function () {
      ...
      assert.equal((results[0].result as any).id, "1");
      assert.include((results[0].result as any).result as string, "key");
      assert.include((results[0].result as any).result as string, "value");
    });
  ...
  describe("tool call chains", function () {
    it("should handle sequential tool calls", async function () {
      ...
      // First LLM call returns first tool call, second returns completion
      let callNumber = 0;
      Object.defineProperty(agent, "mockLLMResponse", {
        get: function () {
          callNumber++;
          if (callNumber === 1) {
            return {
